class SuperGhostBall extends Gasbag;

function bool SameSpeciesAs(Pawn P)
{
	return ( (Monster(P) != None) && (P.IsA('GhostBall') || P.IsA('SuperGhostBall')) );
}

event PostBeginPlay()
{
	Super.PostBeginPlay();
	MyAmmo.ProjectileClass = class'superghostballprojectile';
}
...
{
	SetAnimAction('fly');
	Spawn(class'ghostball',,,location);
}
...
simulated function PlayDying(class<DamageType> DamageType, vector HitLoc)
{
    ...
    bTearOff = true;
    bPlayedDeath = true;
    Spawn(class'XEffects.IonCannonDeathEffect'); //replace xEmitter with the class of the effect you want, for example: class'ShockComboCore' 
...
defaultproperties
{
     ...
     PrePivot=(X=70.000000,Y=0.000000,Z=-50.000000)
     Mass=1000.000000
     drawscale=10
     bDynamicLight=True
     LightType=LT_Steady
