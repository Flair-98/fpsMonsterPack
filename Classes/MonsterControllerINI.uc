...
//=============================================================================
class MonsterControllerINI extends ScriptedController;
 ...
 function bool CanAttack(Actor Other) ///////
{
	...
	return MonsterINI(Pawn).CanAttack(Other);
}

 function StopFiring() ///
{
	MonsterINI(Pawn).StopFiring();
	bCanFire = false;
	bFire = 0;
	bAltFire = 0;
}
 ...
 function bool SetEnemy( Pawn NewEnemy, optional bool bHateMonster ) /////
{
	...
	bNewMonsterEnemy = bHateMonster && (Level.Game.NumPlayers < 4) && !MonsterINI(Pawn).SameSpeciesAs(NewEnemy) && !NewEnemy.Controller.bIsPlayer;
	...
	if ( !bHateMonster && (MonsterINI(Enemy) != None) && NewEnemy.Controller.bIsPlayer )
		return false;
 ...
 /*function bool ClearShot(Vector TargetLoc, bool bImmediateFire)  ////
{
	...
	if ( !bSeeTarget || !FastTrace(TargetLoc , Enemy.Location + Enemy.BaseEyeHeight * vect(0,0,1)) );
		return false;
	if ( (MonsterINI(Pawn).SplashDamage() && (VSize(Pawn.Location - TargetLoc) < MonsterINI(Pawn).GetDamageRadius()))
 ...
 function CheckIfShouldCrouch(vector StartPosition, vector TargetPosition, float probability) //
{
	...
	if ( !Pawn.bCanCrouch || (!Pawn.bIsCrouched && (FRand() > probability))
		|| (Skill < 3 * FRand())
		|| MonsterINI(Pawn).RecommendSplashDamage() )
	{
	...
	projStart = MonsterINI(Pawn).GetFireStart(X,Y,Z);
 ...
 function EnemyChanged(bool bNewEnemyVisible)  ////////////
{
	...
	MonsterINI(Pawn).PlayChallengeSound();
}
 ...
 function WhatToDoNext(byte CallingByte)  //////////
{
	...
	if ( MonsterINI(Pawn).bTryToWalk && (Pawn.Physics == PHYS_Flying) && (ChoosingAttackLevel == 0) && (ChooseAttackCounter == 0) )
		TryToWalk();
	ChoosingAttackLevel++;
	ExecuteWhatToDoNext();
	ChoosingAttackLevel--;
 ...
 function ExecuteWhatToDoNext()  ///////
{
	...
	if ( bPreparingMove && MonsterINI(Pawn).bShotAnim )
	{
 ...
 function FightEnemy(bool bCanCharge) ////////
{
	...
	Target = Enemy;
	if( MonsterINI(Pawn).PreferMelee() || (bCanCharge && bOldForcedCharge) )
	{
		GoalString = "Charge";
		DoCharge();
		return;
	...
	if ( !MonsterINI(Pawn).PreferMelee() && (FRand() > 0.17 * (skill - 1)) && !DefendMelee(enemyDist) )
	{
	...
	if ( !MonsterINI(Pawn).RecommendSplashDamage() && MonsterINI(Pawn).bCanDodge && (FRand() < 0.7) && (FRand()*Skill > 3) )
	{
 ...
 function bool TestDirection(vector dir, out vector pick)  ///////////
{
	...
	HitActor = Trace(HitLocation, HitNormal, MonsterINI(Pawn).Location + pick + 1.5 * MonsterINI(Pawn).CollisionRadius * dir , MonsterINI(Pawn).Location, false);
	if (HitActor != None)
	{
		pick = HitLocation + (HitNormal - dir) * 2 * MonsterINI(Pawn).CollisionRadius;
		if ( !FastTrace(pick, MonsterINI(Pawn).Location) )
			return false;
	}
	else
		pick = MonsterINI(Pawn).Location + pick;

	dist = pick - MonsterINI(Pawn).Location;
	if (MonsterINI(Pawn).Physics == PHYS_Walking)
		dist.Z = 0;
 ...
 function bool NearWall(float walldist) ///////
{
	...
	ViewSpot = MonsterINI(Pawn).Location + MonsterINI(Pawn).BaseEyeHeight * vect(0,0,1);
	...
	{
		FocalPoint = MonsterINI(Pawn).Location + ViewDist;
		return true;
	}
	...
	FocalPoint = MonsterINI(Pawn).Location - LookDir * 300;
	return true;
}
 ...
 function rotator AdjustAim(FireProperties FiredAmmunition, vector projStart, int aimerror) //////
{
	...
	if ( !bClean && (Target == Enemy) && bEnemyInfoValid )
	{
		...
		if ( HitActor != None )
		{
			...
			if ( MonsterINI(Pawn).SplashDamage() && (Skill >= 4) )
			{
			 	HitActor = Trace(HitLocation, HitNormal, FireSpot, ProjStart, false);
				if ( HitActor != None )
 ...
 function ReceiveWarning(Pawn shooter, float projSpeed, vector FireDir) /////////
{
	...
	DodgeSkill = Skill + MonsterINI(Pawn).DodgeSkillAdjust;
	if ( (Pawn.health <= 0) || (DodgeSkill < 4) || (Enemy == None) || !MonsterINI(Pawn).bCanDodge
 ...
 function bool ShouldStrafeTo(Actor WayPoint)///////////
{
	...
	if ( MonsterINI(Pawn).bAlwaysStrafe )
		return true;
	...
	if ( WayPoint == Enemy )
	{
		if ( MonsterINI(Pawn).PreferMelee() )
			return false;
...
{
	if ( (MonsterINI(Pawn).health > 0) && (Damage > 0) && SetEnemy(Other,true) )
		WhatToDoNext(5);
}
...
DoStrafeMove:
	...
	if ( (Enemy == None) || EnemyVisible() || !FastTrace(Enemy.Location, LastSeeingPos) || MonsterINI(Pawn).PreferMelee() || !Pawn.bCanStrafe )
		Goto('FinishedStrafe');
...
RecoverEnemy:
	...
	{
		Pawn.Acceleration = vect(0,0,0);
		if ( MonsterINI(Pawn).SplashDamage() )
		{
			StopFiring();
			Sleep(0.05);
		}
...
Begin:
	...
	FinishRotation();
	if ( MonsterINI(Pawn).HasRangedAttack() && (FRand() < 0.5) && (VSize(Enemy.Location - FocalPoint) < 150) )
...
Begin:
	...
	if ( Target == Enemy )
		...
		FireWeaponAt(Target);
	Sleep(0.1);
	if ( MonsterINI(Pawn).PreferMelee() || (Target == None) || (Target != Enemy) || MonsterINI(Pawn).bBoss )
		WhatToDoNext(35);
	if ( Enemy != None )
		CheckIfShouldCrouch(Pawn.Location,Enemy.Location, 1);
	Focus = Target;
	Sleep(FMax(MonsterINI(Pawn).RangedAttackTime(),0.2 + (0.5 + 0.5 * FRand()) * 0.4 * (7 - Skill)));
	WhatToDoNext(36);
	if ( bSoaking )
