Class RedFireMonster extends Monster;
...
simulated function PostBeginPlay()
{
	...
	{
		Effect = self.spawn(class'MonsterRedFlame', self);
		Effect.SetBase(self);
	}
...
{	
	...
	if ( Controller != None )
	{
		...
		S = Spawn(class'RedFlameShot',,,FireStart,ProjRot);
...
simulated function SpawnGibs(Rotator HitRotation, float ChunkPerterbation)
{
    ...
    while( GibCountTorso-- > 0 )
        SpawnGiblet( Class'RedFlameGib', Location, HitRotation, ChunkPerterbation );
...
defaultproperties
{
     ...
     AmmunitionClass=Class'fpsMonsterPack.RedFlameAmmo'
     ScoringValue=5
