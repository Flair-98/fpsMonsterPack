class GlassProj extends Projectile config(fpsMonsterPack);

var bool bRing,bHitWater,bWaterStart;
var int NumExtraRockets;
var Effects Corona;
var byte FlockIndex;
var GlassProj Flock[2];
...
simulated function PostBeginPlay()
{
	...
	{
		SmokeTrail = Spawn(class'FireBallTrailEmitterG',self,,Location - 40 * Dir, Rotation);
...
simulated function PostNetBeginPlay()
{
	...
	if ( FlockIndex != 0 )
	{
	    ...
	    if ( Flock[1] == None )
	    {
			ForEach DynamicActors(class'GlassProj',R)
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	PlaySound(sound'WraithFireS',,2.1*TransientSoundVolume);
    if ( EffectIsRelevant(Location,false) )
    {
    	Spawn(class'WraithBurst',,,HitLocation + HitNormal*20,rotator(HitNormal));
    	PC = Level.GetLocalPlayerController();
...
defaultproperties
{
     ...
     MyDamageType=Class'fpsMonsterPack.DamTypeGlass'
     ...
     LifeSpan=10.000000
     Texture=Texture'fpsMonsterPack.FireFlareG'
     Skins(0)=Texture'fpsMonsterPack.FireFlareG'
     AmbientGlow=96
